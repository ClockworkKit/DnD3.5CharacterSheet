...
export function abilityCheck(c:Character,a:typeof abilityKeys[number]){return modifier(effectiveScore(c,a))+c.situational+combined(c,['checks','check.'+a]);}
...
export function sheetTotals(c:Character){const mods=Object.fromEntries(abilityKeys.map(a=>[a,modifier(effectiveScore(c,a))])) as Record<typeof abilityKeys[number],number>;const r=racialTraits(c),d=c.defense,size=sizes[c.size][0],auto=c.automation.enabled,ctx=c.automation.context;const free=auto&&movement(c).free,monk=classLevel(c,'monk')+classLevel(c,'psionic-fist'),duelist=classLevel(c,'duelist'),dd=classLevel(c,'dragon-disciple');const denied=auto&&(ctx.flatFooted||['blinded','stunned'].some(id=>effectActive(c,id)));const dex=Math.min(mods.DEX,d.dexCap,denied?0:100);const acComponent=(value:number,target:string,type:string)=>stackBonuses([{value,type,source:'sheet'},...effectTerms(c,target)]);
 ...
 return {mods,size,ac:10+armor+shield+natural+deflection+(denied?0:dodge)+misc+dex+size,touch:10+deflection+(denied?0:dodge)+misc+dex+size,flat:10+armor+shield+natural+deflection+misc+Math.min(0,dex)+size,initiative:mods.DEX+c.initiative+c.situational+(auto?(hasFeat(c,'Improved Initiative')?4:0)+(duelist>=8?4:duelist>=2?2:0)+effectBonus(c,'initiative'):0),grapple:c.bab+mods.STR+sizes[c.size][1]+c.grapple+(c.size==='Colossal'?0:(r?.grapple||0))+c.situational+(auto?(hasFeat(c,'Improved Grapple')?4:0)+effectBonus(c,'grapple'):0),saves,weight:carriedWeight(c)};}
export function skillBonus(c:Character,s:Skill){let total=Math.floor(s.ranks)+modifier(effectiveScore(c,s.ability))+s.misc+racialSkill(c,s)+s.armor*c.defense.checkPenalty+c.situational+skillExtras(c,s.name)+combined(c,['skills','skill.'+s.name]);if(c.automation.enabled&&s.armor===0&&['STR','DEX'].includes(s.ability))total+=equippedArmor(c).nonproficient;return total;}
export function canRollSkill(s:Skill){return !s.trained||s.ranks>=1||s.name.startsWith('Knowledge');}
function weaponContext(c:Character,w:Weapon){const e=equipmentById(w.catalogId);return {weapon:e?.ranged||w.range!=='Melee'&&w.ability==='DEX'?'ranged':'melee',weaponName:w.name.toLowerCase()};}
export function weaponAttack(c:Character,w:Weapon,iteration=0){const t=sheetTotals(c),e=equipmentById(w.catalogId),ctx=c.automation.context,auto=c.automation.enabled,ranged=weaponContext(c,w).weapon==='ranged';let ability=w.ability;if(auto&&!ranged&&hasFeat(c,'Weapon Finesse')&&(e?.hands==='light'||['Rapier','Whip','Chain, spiked','Unarmed strike'].includes(e?.name||''))&&t.mods.DEX>t.mods.STR)ability='DEX';let bonus=c.bab+t.mods[ability]+t.size+w.attack+c.situational-5*iteration;if(!auto)return bonus;const enhancement=Math.max(w.enhancement||0,/bow/i.test(w.name)&&classLevel(c,'arcane-archer')?Math.ceil(classLevel(c,'arcane-archer')/2):0);bonus+=stackBonuses([{value:enhancement,type:'enhancement',source:'weapon'},...effectTerms(c,'attack',weaponContext(c,w))]);if(!enhancement&&w.masterwork)bonus+=1;if(!weaponProficient(c,w))bonus-=4;bonus+=equippedArmor(c).nonproficient+equippedArmor(c).tower;if(hasFeat(c,'Weapon Focus',w.name))bonus++;if(hasFeat(c,'Greater Weapon Focus',w.name))bonus++;if(ctx.flanking&&!ranged)bonus+=2;if(ctx.charge&&!ranged)bonus+=2;if(ctx.defensive==='fighting')bonus-=4;if(hasFeat(c,'Combat Expertise'))bonus-=Math.min(c.bab,5,ctx.combatExpertise);if(hasFeat(c,'Power Attack')&&!ranged)bonus-=Math.min(c.bab,ctx.powerAttack);
 ...
 const race=racialTraits(c);if(race?.id.includes('halfling')&&(e?.thrown||e?.name==='Sling'))bonus++;if(race?.type.includes('dwarf')&&['orc','goblinoid'].includes(ctx.targetRace))bonus++;if(race?.id.includes('gnome')&&race.id!=='deep-gnome'&&['kobold','goblinoid'].includes(ctx.targetRace))bonus++;if(ctx.smite&&classLevel(c,'paladin')&&ctx.targetType==='evil')bonus+=Math.max(0,t.mods.CHA);return bonus;}
export function weaponRange(c:Character,w:Weapon){const e=equipmentById(w.catalogId);const range=e?.range||Number(w.range.match(/\d+/)?.[0]||0);return range*(c.automation.enabled&&hasFeat(c,'Far Shot')?(e?.thrown?2:1.5):1);}
...
export function canCast(p:Caster,s:KnownSpell){return p.mode==='prepared'?s.spent<s.prepared:p.slots[s.slotLevel].used<p.slots[s.slotLevel].max;}
export function spendSpell(p:Caster,s:KnownSpell):Caster {
 if(!canCast(p,s))throw new Error(p.mode==='prepared'?'Prepare an unspent copy of this spell first.':'No daily slots remain at this level.');
...
export function resetDaily(c:Character):Character{return {...c,ancestry:{...c.ancestry,powerPointsSpent:0},psionics:c.psionics.map(p=>({...p,spent:0,focused:false})),casters:c.casters.map(p=>({...p,slots:p.slots.map(x=>({...x,used:0})),spells:p.spells.map(s=>({...s,spent:0}))})),features:c.features.map(f=>({...f,used:0}))};}
