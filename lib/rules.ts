...
export function spellDC(c:Character,p:Caster,s:KnownSpell,reference?:Spell){const school=(reference||s.custom)?.school.split(/[ (]/)[0]||'';return 10+s.level+modifier(effectiveScore(c,findClass(p.casting?.classId||p.name)?.casting?.dcAbility||p.ability))+p.dcExtra+(c.automation.enabled?(hasFeat(c,'Spell Focus',school)?1:0)+(hasFeat(c,'Greater Spell Focus',school)?1:0)+(school==='Illusion'&&racialTraits(c)?.id.includes('gnome')&&racialTraits(c)?.id!=='whisper-gnome'?1:0)+combined(c,['spellDC','spellDC.'+school]):0);}
...
export function resetDaily(c:Character):Character{return {...c,classSystems:{...classSystemsSchema.parse(c.classSystems),choices:c.classSystems.choices.map(s=>({...s,spent:0,granted:false})),pools:Object.fromEntries(Object.entries(c.classSystems.pools).filter(([key])=>key.startsWith('craft:')||key.startsWith('weekly:')))},factotum:{...c.factotum,spent:0,knowledgeUsed:[],pietyUsed:0,dodgeUsed:false,spells:[]},ancestry:{...c.ancestry,powerPointsSpent:0},psionics:c.psionics.map(p=>({...p,spent:0,focused:false})),casters:c.casters.map(p=>({...p,slots:p.slots.map(x=>({...x,used:0})),spells:p.spells.map(s=>({...s,spent:0}))})),features:c.features.map(f=>({...f,used:0}))};}
