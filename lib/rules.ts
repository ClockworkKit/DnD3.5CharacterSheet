import {reservedSlots,illumianDC,illumianExtraDice,illumianSpellNotes,resetRacialDaily} from './racial-abilities.ts';
...
import {equipmentById,equippedArmor,weaponProficient,carriedWeight,carrying,movement,scaleDice} from './equipment.ts';
...
export function weaponAttack(c:Character,w:Weapon,iteration=0){const t=sheetTotals(c),e=equipmentById(w.catalogId),ctx=c.automation.context,auto=c.automation.enabled,ranged=weaponContext(c,w).weapon==='ranged';let ability=w.ability;if(auto&&!ranged&&hasFeat(c,'Weapon Finesse')&&(e?.hands==='light'||['Rapier','Whip','Chain, spiked','Unarmed strike'].includes(e?.name||''))&&t.mods.DEX>t.mods.STR)ability='DEX';let bonus=c.bab+t.mods[ability]+t.size+w.attack+c.situational-5*iteration;if(!auto)return bonus;const enhancement=Math.max(w.enhancement||0,/bow/i.test(w.name)&&classLevel(c,'arcane-archer')?Math.ceil(classLevel(c,'arcane-archer')/2):0);bonus+=stackBonuses([{value:Math.max(enhancement,w.masterwork?1:0),type:'enhancement',source:'weapon'},...effectTerms(c,'attack',weaponContext(c,w))]);if(!weaponProficient(c,w))bonus-=4;bonus+=equippedArmor(c).nonproficient+equippedArmor(c).tower;if(hasFeat(c,'Weapon Focus',w.name))bonus++;if(hasFeat(c,'Greater Weapon Focus',w.name))bonus++;if(ctx.flanking&&!ranged)bonus+=classLevel(c,'swashbuckler')>=8?4:2;if(ctx.charge&&!ranged&&!['fatigued','exhausted','entangled','blinded'].some(id=>effectActive(c,id)))bonus+=2;if(ctx.defensive==='fighting')bonus-=4;if(hasFeat(c,'Combat Expertise'))bonus-=Math.min(c.bab,5,ctx.combatExpertise);if(hasFeat(c,'Power Attack')&&!ranged)bonus-=Math.min(c.bab,ctx.powerAttack);
 ...
 const race=racialTraits(c);if(race?.id.includes('halfling')&&(e?.thrown||e?.name==='Sling'))bonus++;if(race?.type.includes('dwarf')&&race.id!=='earth-dwarf'&&['orc','goblinoid'].includes(ctx.targetRace))bonus++;if(race?.id.includes('gnome')&&!['deep-gnome','air-gnome'].includes(race.id)&&['kobold','goblinoid'].includes(ctx.targetRace))bonus++;if(ctx.smite&&(classLevel(c,'paladin')&&ctx.targetType==='evil'||classLevel(c,'blackguard')>=2&&ctx.targetType==='good'))bonus+=Math.max(0,t.mods.CHA);return bonus;}
export function criticalConfirmation(c:Character,w:Weapon){return weaponAttack(c,w)+effectBonus(c,'critical.confirm');}
...
export function weaponDamage(c:Character,w:Weapon,critical=false){let mod=w.damageAbility==='none'?0:modifier(effectiveScore(c,w.damageAbility));const e=equipmentById(w.catalogId),auto=c.automation.enabled,ctx=c.automation.context;if(auto&&e?.ranged&&!e.thrown&&/bow/i.test(e.name)&&!e.name.includes('Crossbow'))mod=Math.min(modifier(effectiveScore(c,'STR')),e.name.includes('composite')?w.bowStrength||0:0);if(w.damageAbility==='STR'&&mod>0)mod=Math.floor(mod*(usingFlurry(c,w)?1:w.strength==='two'?1.5:w.strength==='off'?.5:1));let bonus=mod+w.damageExtra;
 ...
 if(auto&&classLevel(c,'swashbuckler')>=3&&!ctx.precisionImmune&&equippedArmor(c).category<=1&&(!c.automation.encumbrance||carrying(c).category==='Light')&&(e?.hands==='light'||['Rapier','Whip','Chain, spiked','Unarmed strike'].includes(e?.name||'')))bonus+=Math.max(0,modifier(effectiveScore(c,'INT')));
 let normal=withBonus(parseDiceFormula(weaponDice(c,w)).formula,bonus);if(critical){const n=weaponCritical(c,w).multiplier;normal=parseDiceFormula(normal).terms.map((t,i)=>`${t.sign<0?'-':i?'+':''}${t.kind==='dice'?t.count*n+'d'+t.sides:t.value*n}`).join('');}if(w.extraDamage)normal+='+'+diceFormula(c,w.extraDamage);if(auto&&ctx.sneakAttack&&!ctx.precisionImmune&&(weaponContext(c,w).weapon!=='ranged'||ctx.distance<=30)&&resourceNumbers(c).sneakDice)normal+='+'+resourceNumbers(c).sneakDice+'d6';if(auto&&!ctx.precisionImmune&&(weaponContext(c,w).weapon==='melee'||ctx.distance<=30)){const scout=classLevel(c,'scout'),ninja=classLevel(c,'ninja'),light=equippedArmor(c).category<=1&&(!c.automation.encumbrance||carrying(c).category==='Light');if(ctx.targetFlatFooted&&ninja)normal+='+'+Math.ceil(ninja/2)+'d6';if(ctx.skirmish&&ctx.ownTurn&&scout&&light)normal+='+'+(1+Math.floor((scout-1)/4))+'d6';}if(auto&&illumianExtraDice(c,w))normal+='+'+illumianExtraDice(c,w)+'d6';return parseDiceFormula(normal).formula;}
...
export function spellDC(c:Character,p:Caster,s:KnownSpell,reference?:Spell){const school=(reference||s.custom)?.school.split(/[ (]/)[0]||'';return 10+s.level+modifier(effectiveScore(c,findClass(p.casting?.classId||p.name)?.casting?.dcAbility||p.ability))+p.dcExtra+(c.automation.enabled?(hasFeat(c,'Spell Focus',school)?1:0)+(hasFeat(c,'Greater Spell Focus',school)?1:0)+(school==='Illusion'&&racialTraits(c)?.id.includes('gnome')&&racialTraits(c)?.id!=='whisper-gnome'?1:0)+illumianDC(c,s.level)+combined(c,['spellDC','spellDC.'+school]):0);}
...
export function castingProblem(p:Caster,s:KnownSpell,c?:Character){
 ...
 if(p.mode==='prepared'){
  if(preparedAt(p,s.slotLevel)+slot.used>slot.max)return 'Too many spells are prepared at this level. Adjust preparation to the current slot limit.';
...
export function resetDaily(c:Character):Character{return {...c,racialState:resetRacialDaily(c),classSystems:{...classSystemsSchema.parse(c.classSystems),choices:c.classSystems.choices.map(s=>({...s,spent:0,granted:false})),pools:Object.fromEntries(Object.entries(c.classSystems.pools).filter(([key])=>key.startsWith('craft:')||key.startsWith('weekly:')))},factotum:{...c.factotum,spent:0,knowledgeUsed:[],pietyUsed:0,dodgeUsed:false,spells:[]},ancestry:{...c.ancestry,powerPointsSpent:0},psionics:c.psionics.map(p=>({...p,spent:0,focused:false})),casters:c.casters.map(p=>({...p,slots:p.slots.map((x,i)=>({...x,max:Math.min(10000,x.max+reservedSlots(c,p,i)),used:0})),spells:p.spells.map(s=>({...s,spent:0}))})),features:c.features.map(f=>({...f,used:0}))};}
