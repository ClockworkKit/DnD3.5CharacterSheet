...
export const automationSchema=z.object({
 ...
 levelOrder:z.array(z.string().max(150)).max(100).default([]),
 skillTraining:z.object({version:z.number().int().min(0).max(1).default(0),unassigned:z.record(z.number().min(0).max(1000).multipleOf(.5)).default({})}).default({}),
 history:z.array(z.object({skillRanks:z.record(z.number().min(0).max(1000).multipleOf(.5)).default({}),skillClassOverrides:z.record(z.boolean()).default({}),key:z.string().max(150),intScore:z.number().min(1).max(100),hitDieRoll:z.number().int().min(1).max(100).optional(),skillPoints:z.number().min(0).max(1000).optional()})).max(100).default([]),
