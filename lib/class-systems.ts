...
export function useSystemChoice(c:Character,id:string){const s=c.classSystems.choices.find(s=>s.id===id);if(!s||!lv(c,s.classId))throw new Error('This class choice is not available.');const limit=systemLimits(c,s.classId);if(!limit||!invocationGradeAvailable(c,s)||!systemKinds(c,s.classId).includes(s.kind)||s.level>limit.maxLevel)throw new Error('This choice exceeds the current level limit.');if(s.kind==='maneuver'&&(!s.readied||s.spent||s.classId==='crusader'&&!s.granted))throw new Error('Ready and recover this maneuver before using it.');if(s.spent>=choiceDailyUses(c,s))throw new Error('No daily uses remain.');if(s.kind==='mystery'&&effectiveScore(c,'INT')<10+s.level)throw new Error('Intelligence is too low for this mystery.');if(s.kind==='maneuver'||s.kind==='utterance'||choiceDailyUses(c,s)!==Infinity)s.spent++;if(s.classId==='crusader')s.granted=false;}
