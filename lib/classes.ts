...
export function classTotals(entries:ClassLevel[],ancestry?:Character['ancestry']){const racial=racialHDProgression(ancestry);let {level,bab,fort,ref,will}=racial;const dice:Record<number,number>=racial.level?{8:racial.level}:{};const missing:string[]=[...racial.missing];
...
export function applyClassTotals(c:Character){const t=classTotals(c.classLevels,c.ancestry);if(t.missing.length)throw new Error('Enter custom class totals manually. Unknown table: '+t.missing.join(', '));if(!t.level||t.level>100)throw new Error('Choose between 1 and 100 total class levels.');c.classes=c.classLevels.map(e=>e.name+' '+e.level).join(' / ');c.level=t.level;c.bab=t.bab;c.hitDice=t.hitDice;c.saves.fort.base=t.fort;c.saves.ref.base=t.ref;c.saves.will.base=t.will;const definitions=c.classLevels.map(e=>findClass(e.classId)).filter((d):d is ClassDefinition=>!!d);c.skills.forEach(s=>{s.classSkill=s.classSkillOverride??(isClassSkill(s.name,definitions)||(c.ancestry.raceId==='illumian'&&s.name==='Speak Language')||(c.ancestry.racialHitDice>0&&(c.ancestry.raceId==='lizardfolk'?['Balance','Jump','Swim']:c.ancestry.raceId==='gnoll'?['Climb','Listen','Spot']:[]).includes(s.name)))});}
...
export const castingAbility=(d:ClassDefinition):Ability=>d.casting?.ability||(['wizard','assassin'].includes(d.id)?'INT':['bard','sorcerer'].includes(d.id)?'CHA':'WIS');
export const manifestingAbility=(d:ClassDefinition):Ability=>d.manifesting?.ability||(d.id==='psion'?'INT':d.id==='wilder'?'CHA':'WIS');
export const castingLevel=(d:ClassDefinition,level:number)=>d.casting?.halfLevel||['paladin','ranger'].includes(d.id)?(level<4?0:Math.floor(level/2)):level;
export const manifestingLevel=(d:ClassDefinition,level:number)=>d.id==='soulknife'?0:Math.max(0,level+(d.manifesting?.levelOffset||0));
...
export function makePsionic(def:ClassDefinition,level=3,score=16):Psionic{const row=classRow(def,level),bonus=Math.max(0,Math.floor(Math.floor((score-10)/2)*manifestingLevel(def,level)/2));return {id:crypto.randomUUID(),name:def.name,ability:manifestingAbility(def),level:manifestingLevel(def,level),max:def.id==='soulknife'?2:(row.powerPoints||0)+bonus+(def.manifesting?.bonusPoints||0),spent:0,focused:false,dcExtra:0,notes:def.id==='soulknife'?'Wild Talent supplies 2 power points but no powers or manifester level.':'Choose powers from your class list and discipline. The class table gives powers known and maximum power level.',powers:[],manifesting:{classId:def.id,progression:null,levelAdjustment:0,pointsAdjustment:0,automatic:true}};}
export function copyClassFeatures(c:Character){for(const e of c.classLevels){const d=findClass(e.classId);if(!d)continue;const description=d.levels.filter(r=>r.level<=e.level&&r.special&&r.special!=='—').map(r=>`Level ${r.level}: ${r.special}`).join('\n');const id='class-summary-'+e.id;const feature={id,name:d.name+' class features',kind:'Class feature' as const,description:description+(d.description?'\n\n'+d.description:'')+'\n\nUse the latest improved version of a feature. Bonuses are entered in the relevant sheet fields.\nFull class reference: '+d.source,max:0,used:0,source:d.source};const previous=c.features.find(f=>f.id===id);if(previous){previous.description=feature.description;previous.name=feature.name;}else c.features.push(feature);}}
export function applyStartingClass(c:Character,kind:string,level:number){const def=findClass(kind);if(!def||def.kind==='Prestige')throw new Error('Choose a base class; add prestige classes in Classes.');classRow(def,level);
 ...
 if(def.id!=='fighter'){
  ...
  const primary=def.casting?.ability||def.manifesting?.ability;const fallback=primary==='CHA'?[10,12,14,10,12,16]:primary==='WIS'?[10,12,14,12,16,10]:[10,14,12,16,10,10];
  const [STR,DEX,CON,INT,WIS,CHA]=(spreads[def.id]||fallback);c.scores={STR,DEX,CON,INT,WIS,CHA};c.race='Human';c.languages='Common';c.speed=def.id==='barbarian'?40:def.id==='monk'?30+Math.floor(level/3)*10:30;c.gear=[];c.defense={armor:0,shield:0,natural:0,deflection:0,dodge:0,misc:def.id==='monk'?Math.floor((WIS-10)/2)+Math.floor(level/5):0,dexCap:100,checkPenalty:0,spellFailure:0,sr:0,dr:'',drSources:[],resistances:''};
 ...
 applyClassTotals(c);c.experience=level*(level-1)*500;
 if(!(def.id==='fighter'&&level===3)){c.maxHp=Math.max(level,def.hitDie+Math.ceil((level-1)*(def.hitDie/2+.5))+Math.floor((c.scores.CON-10)/2)*level);c.hp=c.maxHp;}
 if(def.levels[0].slots&&(!['paladin','ranger'].includes(def.id)||level>=4)){const ability=castingAbility(def);c.casters=[makeCaster(def,level,c.scores[ability],c.scores[def.casting?.bonusAbility||ability])];}
 if(def.kind==='Psionic'||def.manifesting)c.psionics=[makePsionic(def,level,c.scores[manifestingAbility(def)])];
 copyClassFeatures(c);
 const addUse=(name:string,max:number,description:string)=>c.features.push({id:crypto.randomUUID(),name,kind:'Class feature',description,max:Math.max(0,max),used:0,source:def.source});
 if(def.id==='barbarian')addUse('Rage',1+Math.floor(level/4),'Track uses here. Enter rage modifiers and duration on the sheet.');
 if(def.id==='bard')addUse('Bardic music',level,'Daily uses. Check the Perform ranks and class level required for each effect.');
 if(def.id==='cleric'||def.id==='paladin'&&level>=4)addUse('Turn or rebuke undead',3+Math.floor((c.scores.CHA-10)/2),'Track daily turning attempts; resolve turning level, check, and damage at the table.');
 if(def.id==='paladin'&&level>=2)addUse('Lay on hands — healing points',level*Math.max(0,Math.floor((c.scores.CHA-10)/2)),'Set Used today to the total number of healing points spent.');
 return c;
}
