...
export const maxDamageReductionSources = 50;
const damageReductionSourceSchema = z.object({
...
const skillSchema = z.object({id:ident,name:short,ability,ranks:z.number().min(0).max(1000).multipleOf(.5),misc:num,trained:z.boolean(),armor:z.number().int().min(0).max(2),classSkill:z.boolean(),classSkillOverride:z.boolean().optional()});
const weaponSchema = z.object({id:ident,name:short,ability,attack:num,damage:z.string().max(80),damageAbility:z.enum(['STR','DEX','none']),strength:z.enum(['one','two','off']),damageExtra:num,crit:short,range:short,ammo:count,notes:z.string().max(2000),catalogId:short.optional(),enhancement:count.optional(),masterwork:z.boolean().optional(),proficiency:z.enum(['auto','yes','no']).optional(),role:z.enum(['main','off']).optional(),attackMode:z.enum(['auto','melee','ranged']).optional(),diceSize:short.optional(),criticalRange:count.optional(),criticalMultiplier:count.optional(),extraDamage:z.string().max(200).optional(),bowStrength:count.optional(),keen:z.boolean().optional()});
...
export const characterSchema = z.object({
  ...
  defense:z.object({armor:num,shield:num,natural:num,deflection:num,dodge:num,misc:num,dexCap:z.number().min(-100).max(100),checkPenalty:z.number().min(-100).max(0),spellFailure:z.number().min(0).max(100),sr:count,dr:short,drSources:z.array(damageReductionSourceSchema).max(maxDamageReductionSources).refine(rows=>new Set(rows.map(row=>row.id)).size===rows.length,'DR source IDs must be unique.').default([]),resistances:z.string().max(2000)}),
...
export type DamageReductionSource = Character['defense']['drSources'][number];
...
export function newCharacter(kind='Fighter',level=3,raceId?:string): Character {
 const c: Character = {racialState:emptyRacialState(),prerequisiteConfirmations:{},classSystems:{choices:[],pools:{},delayedDamage:0},schemaVersion:1,advancementChoices:{},clericSpecialty:'none',factotum:{spent:0,inspirationExtra:0,knowledgeUsed:[],pietyUsed:0,dodgeUsed:false,spells:[]},automation:defaultAutomation(),effects:[],classLevels:[],psionics:[],ancestry:{raceId:'',abilityAdjustments:false,traitBonuses:false,racialHitDice:0,levelAdjustment:0,ignoreLevelAdjustment:false,powerPointsSpent:0,notes:''},name:kind==='Fighter'?'Borin Stoneward':'New '+kind.toLowerCase(),race:kind==='Fighter'?'Dwarf':'Human',classes:kind+' 3',level:3,alignment:'',deity:'',player:'',experience:3000,languages:'Common'+(kind==='Fighter'?', Dwarven':''),appearance:'',size:'Medium',scores:{STR:16,DEX:12,CON:14,INT:10,WIS:10,CHA:8},temps:{STR:0,DEX:0,CON:0,INT:0,WIS:0,CHA:0},hp:26,maxHp:30,tempHp:0,nonlethal:0,hitDice:'3d10',speed:20,initiative:0,bab:3,grapple:0,situational:0,defense:{armor:6,shield:2,natural:0,deflection:0,dodge:0,misc:0,dexCap:1,checkPenalty:-6,spellFailure:40,sr:0,dr:'',drSources:[],resistances:''},saves:{fort:{base:3,misc:0},ref:{base:1,misc:0},will:{base:1,misc:0}},skills:baseSkills.map(([name,ability,trained,armor],i)=>({id:'skill-'+i,name,ability,ranks:0,misc:0,trained,armor,classSkill:kind==='Fighter'&&['Climb','Craft','Handle Animal','Intimidate','Jump','Ride','Swim'].includes(name)})),weapons:[{...newWeapon(),name:'Battleaxe',damage:'1d8',crit:'20 / ×3'}],casters:[],gear:[{id:uid(),name:'Banded mail',qty:1,weight:35,carried:true,equipped:true,notes:'Armor +6; max Dex +1; check −6; arcane failure 35%.'},{id:uid(),name:'Heavy wooden shield',qty:1,weight:10,carried:true,equipped:true,notes:'Shield +2; check −2; arcane failure 15%.'},{id:uid(),name:'Battleaxe',qty:1,weight:6,carried:true,equipped:true,notes:''}],coins:{cp:0,sp:0,gp:50,pp:0},coinWeight:true,features:[],conditions:'',notes:'',background:''};
