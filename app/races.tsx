...
export function Races({c,edit,confirm,roll,spells}:SheetProps&{spells?:import('@/lib/model').Spell[]}){
 ...
 <div className="button-row"><Btn onClick={()=>{edit(d=>copyRacialTraits(d));toast.success('Racial trait reference updated in Feats.')}}>Add / update traits in Feats</Btn><Btn className="quiet danger" onClick={()=>confirm('Detach race reference?','Turn off racial bonuses and remove the attached reference. Keep the race name, entered scores, size, speed, languages, and other notes.',()=>edit(d=>{clearRacialState(d);d.ancestry={...d.ancestry,raceId:'',abilityAdjustments:false,traitBonuses:false,racialHitDice:0,levelAdjustment:0};d.features=d.features.filter(f=>f.id!=='race-reference')}))}>Detach reference</Btn></div>
 <p className="fine">After changing ability adjustments, review maximum HP, bonus spell slots, daily class resources, and class-feature bonuses. With automatic calculations enabled, supported totals update while existing damage and spent uses are preserved.</p>
 </>}
 </Section>
 <RacialAbilities c={c} edit={edit} confirm={confirm} roll={roll} spells={spells}/><Section title="Level adjustment & racial Hit Dice">
 ...
 <Section title="Race library" note={raceCatalog.length+' races and subraces'}><div className="button-row">{['Core','Subraces','Monstrous','Planetouched','Psionic','Other books','Elemental variants','Aquatic variants'].map(g=><Btn key={g} onClick={()=>{setGroup(g);setPreview(null);setOpen(true)}}>{g}</Btn>)}</div><p className="fine">For a race from another book or a homebrew race, enter its name above, add custom Racial traits in Feats, and record adjustments directly on Sheet.</p></Section>
