...
import {applyRacialFireDamage,setSigils,sigilLimit,illumianWord,krauBonus,reserveIllumianSlot,useIllumianWord,swimSpeed,glyphStatus,illumianDC,illumianExtraDice,illumianSpellFormula,advanceRacialRound,wordNames} from '../lib/racial-abilities.ts';
...
test('unprepared-slot words cannot consume prepared copies and failed activation is atomic',()=>{
 const c=create();sig(c,'vaul','krau');const p=c.casters[0];p.spells=[known('magic-missile',1,p.slots[1].max)];const before=structuredClone(c);assert.throws(()=>useIllumianWord(c,{casterId:p.id,level:1}));assert.deepEqual(c,before);p.spells[0].prepared--;useIllumianWord(c,{casterId:p.id,level:1});const q=c.casters[0];assert.equal(q.slots[1].used,1);assert.equal(c.racialState.uses.Vaulkrau,1);q.spells[0].prepared++;assert.match(castingProblem(q,q.spells[0],c),/Too many/);advanceEffects(c);assert.equal(c.racialState.active,null);assert.equal(q.slots[1].used,1);
});
test('turn-funded words spend the shared pool and enforce daily limits',()=>{
 const c=turner();sig(c,'aesh','hoon');focus(c);const w=weapon(),base=weaponDamage(c,w),pool=turn(c);useIllumianWord(c,{turnId:pool.id});assert.equal(turn(c).used,1);assert.notEqual(weaponDamage(c,w),base);assert.equal(effectBonus(c,'damage',{weaponName:'longsword'}),3);useIllumianWord(c,{turnId:pool.id});assert.throws(()=>useIllumianWord(c,{turnId:pool.id}),/daily/);const restored=resetDaily(c);assert.equal(turn(restored).used,0);assert.deepEqual(restored.racialState.uses,{});
});
test('Naenhoon verifies known metamagic, a castable spell and sufficient turning attempts',()=>{
 const c=turner();sig(c,'naen','hoon');const p=c.casters[0],k=known('cure-light-wounds');p.spells=[k];c.features.push({id:crypto.randomUUID(),name:'Empower Spell [Metamagic]',kind:'Feat',description:'',source:'',max:0,used:0});const options={turnId:turn(c).id,targetCasterId:p.id,targetSpellId:k.id,metamagic:'Empower Spell'};useIllumianWord(c,options);assert.equal(turn(c).used,2);assert.match(makeSpellRoll(c,c.casters[0],c.casters[0].spells[0],spells.find(s=>s.id==='cure-light-wounds')).details,/Empower Spell/);const before=structuredClone(c);assert.throws(()=>useIllumianWord(c,{...options,metamagic:'Maximize Spell'}));assert.deepEqual(c,before);
});
test('Vaulnaen spends the donor slot without spending the original preparation',()=>{
 const c=create();sig(c,'vaul','naen');const p=c.casters[0],k=known();p.spells=[k];useIllumianWord(c,{casterId:p.id,level:1,targetCasterId:p.id,targetSpellId:k.id});assert.equal(c.casters[0].slots[1].used,1);assert.equal(c.casters[0].spells[0].spent,0);assert.equal(c.racialState.uses.Vaulnaen,1);
});
test('word riders expire, use insight stacking, distinguish cure/inflict, and avoid multiplying extra dice',()=>{
 const c=turner();sig(c,'hoon','krau');useIllumianWord(c,{turnId:turn(c).id,mode:'turn'});assert.equal(illumianSpellFormula(c,{id:'cure-light-wounds'},'1d8+3'),'1d8+3+1d8');assert.equal(illumianSpellFormula(c,{id:'inflict-light-wounds'},'1d8+3'),'1d8+3');advanceRacialRound(c);assert.equal(c.racialState.active.rounds,1);advanceRacialRound(c);assert.equal(c.racialState.active,null);
 sig(c,'vaul','uur');const p=c.casters[0];useIllumianWord(c,{casterId:p.id,level:2});const w={...weapon(),name:'Unarmed strike'};assert.equal(illumianExtraDice(c,w),2);assert.ok(weaponDamage(c,w,true).endsWith('+2d6'));
});
