...
import {systemWarnings,spendSystemChoice} from '../lib/class-systems.ts';
...
test('invocation selection checks class, grade rather than equivalent level, duplicate and known limit',()=>{
 ...
 assert.equal(least.grade,'least');assert.equal(least.level,3);assert.equal(invocationProblem(c,least),'');learnInvocation(c,least);assert.deepEqual(systemWarnings(c),[]);spendSystemChoice(c,c.classSystems.choices[0].id);assert.equal(c.classSystems.choices[0].spent,0);
...
test('invocations survive saves and daily resets, and level loss flags grade access',()=>{
 ...
 restored.classLevels[0].level=1;assert.ok(systemWarnings(restored).some(w=>w.includes('level limit')));assert.throws(()=>spendSystemChoice(restored,restored.classSystems.choices[0].id),/level limit/);
});
