...
import {factotumNumbers,chooseDilettante,castDilettante,spendInspiration,dilettanteProblem} from '../lib/factotum.ts';
...
test('Arcane Dilettante enforces unique choices, top-level limit, daily use, and inspiration',()=>{
 const c=create({level:4});const spell=spells.find(s=>s.id==='magic-missile'),other=spells.find(s=>s.id==='shield'),cantrip=spells.find(s=>s.id==='detect-magic');chooseDilettante(c,spell);assert.throws(()=>chooseDilettante(c,spell));assert.throws(()=>chooseDilettante(c,other));chooseDilettante(c,cantrip);castDilettante(c,spell.id);assert.equal(c.factotum.spent,1);assert.throws(()=>castDilettante(c,spell.id));c.factotum.spent=0;assert.throws(()=>castDilettante(c,spell.id));spendInspiration(c,3);assert.throws(()=>castDilettante(c,cantrip.id));assert.equal(c.factotum.spells[1].spent,false);assert.match(dilettanteProblem(c,{...cantrip,components:'V, XP'}),/XP/);const reset=resetDaily(c);assert.equal(reset.factotum.spells.length,0);assert.equal(factotumNumbers(reset).remaining,3);
});
