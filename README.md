...
### Feat and prestige prerequisites
...
These checks govern new selections. Existing character choices are preserved, and prestige classes already recorded may continue advancing. They do not enforce feat-slot budgets, disable existing feats after losing prerequisites, waive ordinary prerequisites for special bonus-feat selections, or enforce base-class alignment/multiclass restrictions. Use the existing custom/manual controls for DM-approved exceptions. New browser controls still require a live acceptance check after deployment.

### Unusual racial rules

The Race tab now includes Illumian sigil/word controls, Aquatic Kobold, and the ten Unearthed Arcana elemental variants: Air Gnome/Goblin, Earth Dwarf/Kobold, Fire Elf/Half-Elf/Hobgoblin, and Water Half-Orc/Halfling/Orc. These variants inherit their base race except for the listed replacements. Aquatic Kobold is the environmental aquatic variant, not an elemental creature.

- Illumians: six sigils, the class-level-2 second-sigil rule, suppression, check/initiative bonuses, capped Krau caster levels, Strength/Dexterity bonus-slot choices, Speak Language class-skill access, shadow saves, and glyphic resonance. Human subtype does not grant human bonus feats/skills.
- All fifteen Illumian word combinations have controls. Numerical riders, slot reservations, turning/slot costs, daily limits, and durations are tracked. Reserved slots stay unavailable while sigils are suppressed and return on daily reset/preparation. Slot-funded words cannot consume prepared copies. Vaulnaen sends the selected prepared spell through the existing roll destination and spends only its donor slot after a successful handoff.
- Swimming: racial speed and +8 checks, take-10 support, and straight-line run guidance. Aquatic Kobold has 40-foot swimming and water-only breathing; its out-of-water breath counter does not assume amphibiousness. Its default LA is zero; the source's campaign-dependent optional +1 uses the existing LA setting.
- Elemental controls: opposing-subtype attacks, magical-save penalties, stability, stonework, inherited-trait exceptions, and fire resistance. The fire-damage helper applies the greater of racial/entered resistance and consumes temporary HP first. Fire Half-Elf and Water Half-Orc do not gain the full elemental package.

Player choices still matter: select the incoming effect/opponent, trigger qualifying effects, advance timers at the specified turn boundary, and resolve suffocation, metamagic, and other target-dependent outcomes. Naenhoon verifies its feat/spell and pays turning costs; its metamagic effect remains in the spell's existing manual controls. Naenaesh identifies Still Spell access on the spell card. This is not a general spell-effect or underwater-combat simulator. The normal save button consumes Vaulkrau after a successful roll handoff; loose-dice/custom rolls require ending the effect manually. Existing custom effects that duplicate a newly automated bonus should be removed.

Sources: [Illumians — Races of Destiny](https://srd.dndtools.org/srd/races/racesRod.html), [elemental variants](https://srd.dndtools.org/srd/variant/unearthedElementalVariants.html), and [aquatic variants](https://srd.dndtools.org/srd/variant/unearthedEnvironmentalVariants.html), using the project's Ultimate SRD. Rebuild the additional racial catalog entries with `python scripts/add-unusual-races.py` after importing the base races. New controls need live browser acceptance after deployment.

### Unified Magic tab and invocations

Spells, psionic powers, recorded spell-like abilities, Factotum resources, and magical class systems share **Magic**. Only applicable sections open by default. **Add another magic tradition** exposes casting/psionic setup and custom spell-like abilities. Feats & features can move any non-feat ability into Magic; its existing counter and save identity are retained. Martial systems and auras stay in Classes. Irrelevant racial environment controls are hidden.

**Browse invocations** imports from 86 Warlock and 30 Dragonfire Adept list entries in the Ultimate SRD. The picker filters by class, grade, duplicate selections, and known-invocation capacity. Grade is saved separately from equivalent spell level: some least invocations are level 3, and some greater invocations have lower equivalent levels. Saved cards show source links, caster level, applicable save DC, and at-will usage. They do not spend ordinary spell slots. Individual effects, shapes/essences, and targeting still need resolution from the source. Old custom choices remain editable, with grade restrictions checked manually unless a grade was recorded.
